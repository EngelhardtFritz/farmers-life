...
public class WinePressScreen extends ContainerScreen<WinePressContainer> {
    public static final ResourceLocation WINE_PRESS_GUI_TEXTURE = new ResourceLocation(FarmersLife.MOD_ID, "textures/gui/container/wine_press.png");
    public WinePressTileEntity winePressTileEntity;

    private static final Rectangle2d GUI_PROGRESS = new Rectangle2d(0, 166, 14, 22);
    private static final Rectangle2d GUI_OUTPUT_TANK = new Rectangle2d(118, 18, 13, 49);

    public WinePressScreen(WinePressContainer screenContainer, PlayerInventory inv, ITextComponent titleIn) {
        super(screenContainer, inv, titleIn);
        this.winePressTileEntity = screenContainer.winePressTileEntity;
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        int totalSteps = this.winePressTileEntity.getProcessStepTotal();
        ...
        this.blit(this.guiLeft + 79, this.guiTop + 32, GUI_PROGRESS.getX() + progressionIndex * GUI_PROGRESS.getWidth(), GUI_PROGRESS.getY(), GUI_PROGRESS.getWidth(), GUI_PROGRESS.getHeight());

        FluidRenderHelper.drawFluidTank(this.container.getFluidStackForTank(WinePressTileEntity.OUTPUT_TANK_SLOT),
                ContainerHelper.getScaledInputTankHeight(this.winePressTileEntity.getTank(WinePressTileEntity.OUTPUT_TANK_SLOT), GUI_OUTPUT_TANK.getHeight()),
    ...
    private int getPressProgressIndex(int currentSteps, int totalSteps) {
        float stepWidth = totalSteps / 10.0F;
        for (int i = 0; i < 10; i++) {
    ...
    protected void renderHoveredToolTip(int mouseX, int mouseY) {
        if (isPointInRegion(GUI_OUTPUT_TANK.getX(), GUI_OUTPUT_TANK.getY(), GUI_OUTPUT_TANK.getWidth(), GUI_OUTPUT_TANK.getHeight(), mouseX, mouseY)) {
            FluidTank tank = this.container.getFluidTankForSlot(WinePressTileEntity.OUTPUT_TANK_SLOT);
