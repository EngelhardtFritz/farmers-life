...
public class BoilingRecipe implements IBoilingCauldronRecipe, IRecipe<RecipeWrapper> {
    public static final BoilingRecipeSerializer BOILING_RECIPE_SERIALIZER = new BoilingRecipeSerializer();
    public static final ResourceLocation BOILING_ID = new ResourceLocation(FarmersLife.MOD_ID, "boiling_recipe");
    ...
    public BoilingRecipe(ResourceLocation recipeId, Ingredient inputIngredient, int inputIngredientCount, FluidStack inputFluidStack, ItemStack outputItemStack, FluidStack outputFluidStack, int processTime, boolean requiresLid) {
    ...
    public IRecipeSerializer<?> getSerializer() {
        return ModRecipeSerializer.BOILING_RECIPE_SERIALIZER.get();
    }

    @Override
    public IRecipeType<?> getType() {
        return ModRecipeSerializer.BOILING_RECIPE_TYPE;
    }
    ...
    public static class BoilingRecipeSerializer extends ForgeRegistryEntry<IRecipeSerializer<?>> implements IRecipeSerializer<BoilingRecipe> {
        public static List<Fluid> fluidInputList = new ArrayList<>();
        public static List<Item> itemInputList = new ArrayList<>();

        @Override
        public BoilingRecipe read(ResourceLocation recipeId, JsonObject json) {
            ...
            if (ingredientFluidObject != null) {
                ...
                if (ingredientFluid == null) {
                    throw new IllegalArgumentException("[BoilingRecipeSerializer::read] Expected resource location of ingredient fluid but got: " + inputFluidLocation);
                }
            ...
            if (inputIngredient == Ingredient.EMPTY && inputFluidStack == FluidStack.EMPTY) {
                throw new IllegalArgumentException("[BoilingRecipeSerializer::read] Expected ingredient or fluid as input but got null!");
            }
            ...
            if (outputFluidStackObject != null) {
                ...
                if (outputFluid == null) {
                    throw new IllegalArgumentException("[BoilingRecipeSerializer::read] Expected resource location of output fluid but got: " + outputFluidLocation);
                }
            ...
            if (outputItemStack.isEmpty() && outputFluidStack.isEmpty()) {
                throw new IllegalArgumentException("[BoilingRecipeSerializer::read] Expected output fluid or item stack to be not null!");
            }

            final int processTime = JSONUtils.getInt(json, "processTime");
            final boolean requiresLid = JSONUtils.getBoolean(json, "requiresLid");
            return new BoilingRecipe(recipeId, inputIngredient, inputIngredientCount, inputFluidStack, outputItemStack, outputFluidStack, processTime, requiresLid);
        }

        @Nullable
        @Override
        public BoilingRecipe read(ResourceLocation recipeId, PacketBuffer buffer) {
        ...
        @Override
        public void write(PacketBuffer buffer, BoilingRecipe recipe) {
